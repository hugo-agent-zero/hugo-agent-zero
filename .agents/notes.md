...
- **Next:** [#70](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/70) — measure/type A/B/C when we code. Names simmer ~24h: layouts tap **`haz_tbd`** (vs `haz_dc`); today’s `haz_tbd` → **`haz_do`** toolbox. No rename until it still sings ([#86](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/86) / [#87](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/87)). Do **not** merge [core #51](https://github.com/hugo-agent-zero/hugo-agent-zero-core/pull/51) / [#80](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/80). Keys simmer [#89](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/89). Headlines parked [#90](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/90). How To [#13](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/13) still pending. Width map parked [#72](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/72).
- ~~**Next (tomorrow):** [#70](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/70) — measure/type refactor. Column max in **px** (A). Font max (B) is the size that puts ~65 of a letter/digit sample in A (not `ch` / zeros). Min is `100%` (C). Slope C→B. No `type`/`sweet` delta. Defaults for our face; swap face → retune A/B/C. `haz_img`: `auto` + aspect-ratio frame is the height lock — do **not** merge [core #51](https://github.com/hugo-agent-zero/hugo-agent-zero-core/pull/51) / [#80](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/80). Keys simmer [#89](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/89). Headlines parked [#90](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/90). How To [#13](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/13) still pending. Width map parked [#72](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/72).~~ Superseded 2026-09-26 morning — #70 still next to code; 30 min was shortcode names.
...
- ~~**Parked (simmer):** [#19]… **Next sitting (when back):** Pre-#21…~~ Superseded 2026-09-02 EOD — #43 Phase 2 is the jump-in.
...
### 2026-09-26 (morning) — shortcode names; simmer

- ~30 min. No code. `haz_img` stays.
- Layouts tap (today `{{< haz key="…" >}}`): **`haz_dc` or `haz_tbd`**. Tilt: **`haz_tbd`**. Declared spot; fill from the graph; keep filling it. We will explain it — nobody else in Hugo has this. Fontaines DC can sit down.
- Toolbox locked: one shortcode, not 4–6 helpers. Not WordPress; sweet spot; we say no. Today’s `haz_tbd` → **`haz_do`**. First blade `add="spacer"`. Attr name is the verb so a future blade is not forced to be an “add.” One verb per call.
- Registry later: yaml maps the blade to an HTML partial. Child can add tools. Core ships the handful. Implementation TBD. Smart even if only we extend. Not a mall we merge from the internet.
- Rename order when it still sings: helper → `haz_do` first (free the name), then `haz` → `haz_tbd`. FF `v1.x.x` before Pages. [#86](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/86) / [#87](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/87).
- Dropped from the lead: `util`, `sak`, `haz_add what=`, `add add=`. `#70` A/B/C still the next coding sitting.

**Plans:**
- [shortcode name simmer](plans/shortcode_name_simmer_4791894f.plan.md) — HQ #86 / #87; **parked** (~24h mouthfeel)
...
**Sitting:** ISP cutover day — limited hours. Still made progress on showable-alpha path.

**#13 haz_img:** Full design pass (Plan mode). Paired shortcode wraps author `<img>`; injects `srcset` + `sizes` + derived window class. Measure-first (`content_width` / `--haz_measure`, ≤45rem): windows and variant px factors trickle from that. Markdown `![]()` stays dumb/opt-out. Sleep on the plan; build when fresh. GOTO: [Eric Portis — srcset/sizes](https://ericportis.com/posts/2014/srcset-sizes/).

**Not done:** Social icon SVG chrome **fills** still wrong — mop **tomorrow** (same family as earlier search-btn fill fixes).

**Still queued:** [#49](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/49) demo content pairs with `#13`.

**Plans:**
- [haz_img shortcode](plans/haz_img_shortcode_4d1f620b.plan.md) — HQ [#13](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/13); status **parked** (sleep / build next)

### 2026-09-11 — Prioritize HAZ showable alpha; #49 demo content

**Bigger picture:** HAZ stays the priority. Goal is something Mark can hand a friend / colleague — **alpha-ish**, not perfect. Demo isn’t there yet; random content undercuts the kit.

**Tickets:** Opened [#49](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/49) (demo content refresh). Pairs with existing [#13](https://github.com/hugo-agent-zero/hugo-agent-zero/issues/13) (responsive `srcset` + `sizes` shortcode). Rough package ~12h when both land; weekend hours welcome.

**Next:** #13 and/or #49 when back — even a couple hours.

### 2026-09-03 — #43 Phase 2–3 + meta columns shipped; next = HAZ.com content

*Status 2026-09-11:* Jump-in reframed — showable alpha via #13 + #49 before deep HAZ.com content (see Meeting that date).
...
**Paused:** Mark errand; tomorrow = Phase 2.

**Plans:**
- [Blog list / #43](plans/blog_list_main_styling_bafcd32b.plan.md) — Phase 1 shipped; Phase 2 + short_desc; in progress
...
**Paused:** Mark simmering on the plan before putting it in play.
